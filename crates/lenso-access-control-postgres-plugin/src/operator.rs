...
/// Explicit schema administration for Access Control.
#[derive(Clone, Copy, Debug, Default)]
pub struct AccessControlOperator;

impl AccessControlOperator {
    /// Creates a missing managed schema and installs the complete authored plan.
    pub async fn setup(
        database_url: &str,
        schema: &str,
    ) -> Result<SetupOutcome, AccessControlOperatorError> {
    ...
    pub async fn upgrade(
        database_url: &str,
        schema: &str,
    ) -> Result<UpgradeOutcome, AccessControlOperatorError> {
        Ok(SchemaOperator::connect(database_url, schema_plan(schema)?)
            .await?
            .upgrade()
            .await?)
...
/// Failure from an explicit Access Control schema workflow.
#[derive(Debug, Error)]
pub enum AccessControlOperatorError {
    #[error(transparent)]
